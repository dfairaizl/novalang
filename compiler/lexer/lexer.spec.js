...
describe("Lexer", () => {
  ...
  describe("whitespace", () => {
    it("skips white space before tokens", () => {
      const lex = new Lexer("    +");
      expect(lex.nextToken()).toStrictEqual(new OperatorToken("+"));
    });

    it("skips white space between tokens", () => {
      const lex = new Lexer("1   +   2");

      expect(lex.nextToken()).toStrictEqual(
        new NumberToken("1", { kind: "Int" })
      );
      expect(lex.nextToken()).toStrictEqual(new OperatorToken("+"));
      expect(lex.nextToken()).toStrictEqual(
        new NumberToken("2", { kind: "Int" })
  ...
  describe("operators", () => {
    ...
    it("does not lex `*,`", () => {
      const lex = new Lexer("*,");
      expect(lex.nextToken()).toStrictEqual(new OperatorToken("*"));
      expect(lex.nextToken()).toStrictEqual(new PunctuatorToken(","));
    });
  });
  ...
  describe("numbers", () => {
    ...
    it("lexes `1.0`", () => {
      ...
      expect(lex.nextToken()).toEqual(
        new NumberToken("1.0", { kind: "Float" })
      );
    });
    ...
    it("lexes `3.14159`", () => {
      ...
      expect(lex.nextToken()).toEqual(
        new NumberToken("3.14159", { kind: "Float" })
      );
    });
  });
  ...
  describe("booleans", () => {
    it("lexes true boolean literal", () => {
      ...
      expect(lex.nextToken()).toStrictEqual(new BooleanToken("true"));
    });

    it("lexes false boolean literal", () => {
      const lex = new Lexer("false");
      expect(lex.nextToken()).toStrictEqual(new BooleanToken("false"));
    });
  });
  ...
  describe("strings", () => {
    it("lexes string literals from single quotes", () => {
      ...
      expect(lex.nextToken()).toStrictEqual(new StringToken("hello world"));
    });
  ...
  describe("keywords", () => {
    ...
    it("lexes `else`", () => {
      const lex = new Lexer("else");
      const token = lex.nextToken();

      expect(token).toBeInstanceOf(KeywordToken);
      expect(token).toEqual(new KeywordToken("else"));
    });
    ...
    it("lexes `new`", () => {
      const lex = new Lexer("new");
      const token = lex.nextToken();

      expect(token).toBeInstanceOf(KeywordToken);
      expect(token).toEqual(new KeywordToken("new"));
    });
    ...
    it("lexes `this`", () => {
      ...
      expect(token).toBeInstanceOf(KeywordToken);
      expect(token).toEqual(new KeywordToken("this"));
    });
  ...
  describe("identifiers", () => {
    it("lexes `something`", () => {
      ...
      expect(lex.nextToken()).toStrictEqual(new IdentifierToken("something"));
    });

    it("lexes `c1`", () => {
      const lex = new Lexer("c1");
      expect(lex.nextToken()).toStrictEqual(new IdentifierToken("c1"));
    });

    it("lexes `key.value` as separate tokens", () => {
      const lex = new Lexer("key.value");
      expect(lex.nextToken()).toStrictEqual(new IdentifierToken("key"));
      expect(lex.nextToken()).toStrictEqual(new OperatorToken("."));
      expect(lex.nextToken()).toStrictEqual(new IdentifierToken("value"));
    });
  });
});

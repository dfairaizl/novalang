...
function analyse(node, env, nonGeneric) {
    if (node instanceof Id) {
        ...
        let funcType = analyse(node.func, env, nonGeneric), argType = analyse(node.arg, env, nonGeneric), retType = new TypeVariable();
        unify(FunctionType(argType, retType), funcType);
        return retType;
    }
    else if (node instanceof Lambda) {
        let argType = new TypeVariable(), newEnv = env.extend(node.arg, argType), newGeneric = new Set(Array.from(nonGeneric).concat(argType)), retType = analyse(node.body, newEnv, newGeneric);
        return FunctionType(argType, retType);
    }
    else if (node instanceof Let) {
        let valType = analyse(node.value, env, nonGeneric), newEnv = env.extend(node.variable, valType);
        return analyse(node.body, newEnv, nonGeneric);
    }
    else if (node instanceof Letrec) {
        let newType = new TypeVariable(), newEnv = env.extend(node.variable, newType), newGeneric = new Set(Array.from(nonGeneric).concat(newType)), valType = analyse(node.value, newEnv, newGeneric);
...
function unify(type1, type2) {
    let t1 = prune(type1), t2 = prune(type2);
...
// ...
const type1 = new TypeVariable(), type2 = new TypeVariable(), type3 = new TypeVariable();
